...
RUN composer install --optimize-autoloader --no-dev --no-interaction --prefer-dist \
    ...
    if [ ! -d "vendor" ]; then echo "ERROR: vendor directory not created!"; exit 1; fi

# 5. Pre-generate Wayfinder types for the Node build stage
RUN mkdir -p resources/js/actions resources/js/routes \
    && php artisan wayfinder:generate --with-form --no-interaction || true
...
# Copy pre-built dependencies and generated files from base stage
...
# Set WAYFINDER_SKIP_GENERATION to prevent the plugin from trying to run PHP
ENV WAYFINDER_SKIP_GENERATION=true
